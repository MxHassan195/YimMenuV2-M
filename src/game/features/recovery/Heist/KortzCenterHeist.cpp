...
namespace YimMenu::Features
{
  namespace KortzCenterHeist
  {
    ...
    static BoolCommand _KortzCenterManchezYellow{"kortzcenterheistmanchezyellow", "Manchez (Yellow)", "Enable Yellow Manchez", true};
    ...
    class Setup : public Command
    {
      ...
      virtual void OnCall() override
      {
        ...
        // Weapon Loadout Handling (0 = All, 1 = Street, 2 = Security, 3 = Military)
        ...
        // Manchez Vehicle Selections (Multi-Select Support)
        bool manchez = _KortzCenterManchez.GetState();
        if (!(manchez && _KortzCenterManchezRed.GetState()))    generalBits &= ~(1 << 17); // Red Manchez
        if (!(manchez && _KortzCenterManchezBlue.GetState()))   generalBits &= ~(1 << 18); // Blue Manchez
        if (!(manchez && _KortzCenterManchezGreen.GetState()))  generalBits &= ~(1 << 19); // Green Manchez
        if (!(manchez && _KortzCenterManchezYellow.GetState())) generalBits &= ~(1 << 20); // Yellow Manchez
